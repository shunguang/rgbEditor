...
std::string app::my_av_err2str(int ret)
{
char buf[256];
...
std::string app::my_av_ts2str(const int64_t x)
{
char buf[64];
...
std::string app::my_av_ts2timestr(const int64_t ts, const AVRational *tb)
{
char buf[256];
...
void app::log_packet(const AVFormatContext *fmt_ctx, const AVPacket *pkt)
{
...
void app::dumpRaw(const char *f, const uint8 *buf, const uint64 sz)
{
FILE *fid = fopen( f, "wb");
if (!fid) {
dumpLog("Could not open destination file %s", f);
return;
}

fwrite(buf, sz, 1, fid);
fclose(fid);
dumpLog("Note: Wrote a file:%s", f);
}

void app::raw2wav(const std::string &rawPath, const bool isDeletedRaw)
{
...
dumpLog("Note: create a file: %s", out.c_str());
}

void app::loadRaw(const char *f, std::vector<char> &buf)
{
...
uint32_t app::ppm_save(ppm_image *img, FILE *outfile) {
size_t n = 0;
...
uint32_t app::yuv420p_save(const uint8 *yuv420pRaw, const int sz, const int w, const int h, const char *fname)
{
...
void app::dumpFrmImg( const RawFrm &x, const std::string &folder, const std::string &tag)
{
...
void app::resizeYuvVideoFrm(RawFrm &dst, const uint8 *srcBuf, const int srcW, const int srcH,  const enum libyuv::FilterMode flag)
{
...
void app::resizeYuvVideoFrm(RawFrm &dst, const RawFrm &src, const enum libyuv::FilterMode flag)
{
const int dstW = dst.w_;
const int dstH = dst.h_;
#if _DEBUG
...
void app::resizeYuvVideoFrms(const int srcW, const int srcH, const std::vector<RawFrm> &vSrc, const int desW, const int desH, std::vector<RawFrm> &vDes)
{
...
void app::resizeYuvVideoFrms(const int srcW, const int srcH, const std::vector<RawFrmPtr> &vSrc, const int desW, const int desH, std::vector<RawFrmPtr> &vDes)
{
...
bool app::saveAudioStreamInfo(const std::string &filePath, const MyAudioStreamInfo &info)
{
std::ofstream myFile(filePath, std::ios::out | std::ios::binary);
if (!myFile.is_open()) {
dumpLog("ProjProfile::saveVideoStreamInfo(): cannot open file: %s", filePath.c_str());
return false;
}
...
bool app::saveVideoStreamInfo(const std::string &filePath, const MyVideoStreamInfo &info)
{
std::ofstream myFile(filePath, std::ios::out | std::ios::binary);
if (!myFile.is_open()) {
dumpLog("ProjProfile::saveVideoStreamInfo(): cannot open file: %s", filePath.c_str());
...
//static funcs
bool app::loadVideoStreamInfo(const std::string &fpath, MyVideoStreamInfo &info)
{
ifstream myFile(fpath, ios::in | ios::binary);
if (!myFile.is_open()) {
dumpLog("ProjProfile::loadVideoStreamInfo(): cannot open file: %s", fpath.c_str());
return false;
}
...
bool app::loadAudioStreamInfo(const std::string &fpath, MyAudioStreamInfo &info)
{
ifstream myFile(fpath, ios::in | ios::binary);
if (!myFile.is_open()) {
dumpLog("ProjProfile::loadAudioStreamInfo(): cannot open file: %s", fpath.c_str());
return false;
}
