...
void RunMgr::setCfg(AppCfgPtr  &cfg)
{
...
dumpLog("RunMgr::dexInParallel(): All files are Dexed!");
...
dumpLog("RunMgr::anaInParallel(): All files are analyzed!");
}
...
dumpLog("game: %s", game.name.c_str());
...
dumpLog("RunMgr::startAnaAudio()--AA, isPauseJob=%d", isPauseJob());
setTaskId(PRO_MSG_ANA_AUDIO);
assert(true == isPauseJob());
startJob();
dumpLog("RunMgr::startAnaAudio()--BB, isPauseJob=%d", isPauseJob()); ;
}
...
dumpLog(outAudioInfo.toString("outAudioInfo="));
...
dumpLog(outVideoInfo.toString("outVideoInfo="));
...
dumpLog("void RunMgr::writeOuputVideo(): well done!");
}
