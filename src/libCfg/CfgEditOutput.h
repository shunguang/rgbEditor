...
#ifndef __CFG_EDIT_OUTPUT_H__
#define __CFG_EDIT_OUTPUT_H__

#include "CfgOutput.h"
namespace app {
	class CfgEditOutput : public CfgOutput {
	public:
		CfgEditOutput();
		CfgEditOutput(const CfgEditOutput &x) = default;
		CfgEditOutput(CfgEditOutput &&x) = default;
		virtual ~CfgEditOutput();
		CfgEditOutput& operator = (const CfgEditOutput &x) = default;
		...
		void setPrms(const CfgEditOutput &x) {
			*this = x;
		}
	...
	typedef std::shared_ptr<CfgEditOutput>	CfgEditOutputPtr;
}
#endif
