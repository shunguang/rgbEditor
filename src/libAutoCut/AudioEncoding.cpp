...
if (!codec) {
dumpLog("AudioEncoding::resetCodec(): Codec not found");
return false;
}

m_ctx = avcodec_alloc_context3(codec);
if (!m_ctx) {
dumpLog("AudioEncoding::resetCodec(): Could not allocate audio codec context");
return false;
}
...
dumpLog("AudioEncoding::resetCodec(): Encoder does not support sample format %s", av_get_sample_fmt_name(m_ctx->sample_fmt));
return false;
}
...
dumpLog("AudioEncoding::resetCodec(): Could not open codec");
return false;
}
...
if (!m_pkt) {
dumpLog("AudioEncoding::resetCodec(): could not allocate the packet");
return false;
}
...
if (!m_frame) {
dumpLog("AudioEncoding::resetCodec(): Could not allocate audio frame");
return -1;
}
...
if (ret < 0) {
dumpLog("AudioEncoding::encode_audio(): Could not allocate audio data buffers");
return -1;
}
...
{
if (!m_isInit) {
dumpLog("AudioEncoding::encode_audio(): Codec was not init!");
return -1;
}
...
if (!f) {
dumpLog( "AudioEncoding::encode_audio(): Could not open %s", outFile.c_str());
return -1;
}
...
if (ret < 0) {
dumpLog("AudioEncoding::encode_audio(): Could not run av_frame_make_writable()!");
return -1;
}
for (int j = 0; j < nChs; ++j) {
...
if (nFrms <= 0) {
dumpLog("AudioEncoding::encode_audio_planarFrms(): <vPlanarFrms> is empty!");
return -1;
}
uint64 frmSzInBytes = getFrmSize();
uint64 bufSz = vPlanarFrms[0]->sz_;
if (frmSzInBytes != bufSz) {
dumpLog("AudioEncoding::encode_audio_planarFrms(): frm size does not match, frmSzInBytes=%d, bufSz=%d", frmSzInBytes, bufSz);
return -1;
}
...
f = fopen(outFile.c_str(), "wb");
if (!f) {
dumpLog("AudioEncoding::encode_audio_planarFrms(): Could not open %s", outFile.c_str());
return -1;
}
...
if (ret < 0) {
dumpLog("AudioEncoding::encode_audio_planarFrms(): Could not run av_frame_make_writable()!");
return -1;
}
...
f = fopen(outFile.c_str(), "wb");
if (!f) {
dumpLog("AudioEncoding::encode_audio_nonPlanarFrms(): Could not open %s", outFile.c_str());
return -1;
}
...
int ret = av_frame_make_writable(m_frame);
if (ret < 0) {
dumpLog("AudioEncoding::encode_audio_nonPlanarFrms(): Could not run av_frame_make_writable()!");
return -1;
}
...
dumpLog("AudioEncoding::check_sample_fmt(): AVSampleFormat=%d (%s)", *p, av_get_sample_fmt_name(*p) );
if (*p == sample_fmt)
return 1;
p++;
}
...
if (ret < 0) {
dumpLog( "Error sending the frame to the encoder");
return -1;
}
...
dumpLog( "Error encoding audio frame");
return -1;
}
