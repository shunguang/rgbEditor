...
#ifndef __CFG_COMBINE_OUTPUT_H__
#define __CFG_COMBINE_OUTPUT_H__

#include "CfgOutput.h"
namespace app {
	class CfgCombineOutput : public CfgOutput {
	public:
		CfgCombineOutput();
		CfgCombineOutput(const CfgCombineOutput &x) = default;
		CfgCombineOutput( CfgCombineOutput &&x) = default;
		virtual ~CfgCombineOutput();

		CfgCombineOutput& operator = (const CfgCombineOutput &x) = default;

		virtual boost::property_tree::ptree toPropertyTree();
		virtual void fromPropertyTree(const boost::property_tree::ptree &pt);

		virtual void genOutputVideoInfo();

		void setPrms(const CfgCombineOutput &x) {
			*this = x;
		}
	...
	typedef std::shared_ptr<CfgCombineOutput>	CfgCombineOutputPtr;
}
#endif
