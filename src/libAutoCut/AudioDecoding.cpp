...
if (ret < 0) {
dumpLog("AudioDecoding::decode_audio(): cannot ope file : %s", outputFile.c_str() );
return -1;
}
...
if (!codec) {
dumpLog( "Codec not found");
return -1;
}

parser = av_parser_init(codec->id);
if (!parser) {
dumpLog( "Parser not found");
return -1;
}

c = avcodec_alloc_context3(codec);
if (!c) {
dumpLog( "Could not allocate audio codec context");
return -1;
}

/* open it */
if (avcodec_open2(c, codec, NULL) < 0) {
dumpLog( "Could not open codec");
return -1;
}

fp = fopen(infilename, "rb");
if (!fp) {
dumpLog( "Could not open %s\n", infilename);
return -1;
}
...
dumpLog( "Could not allocate audio frame");
return -1;
}
}
...
if (ret < 0) {
dumpLog( "Error submitting the packet to the decoder");
return -1;
}
...
int app::audio_convert_nonPlanar2nonPlanar(std::vector<uint8> &desRaw, const enum AVSampleFormat desFmt, const int desChannels, const int desSmpRate,
...
int app::audio_convert_nonPlanar2planar(std::vector<std::vector<uint8>> &des, const std::vector<uint8> &src, const int nChannels, const int nBytes4Smp)
{
