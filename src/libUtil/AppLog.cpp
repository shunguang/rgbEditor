...
//----------------- global funcs ---------------------
void app::dumpLog( const std::string &x, ... )
{
	app::dumpLog( x.c_str() );
}
...
//-------------------------------
void app::startLogThread( const std::string &logFilePath, const bool showInConsole, const bool dumpLog)
{
...
void  app::appExit(const int flag)
{
	if (flag != 0) {
...
void  app::appExit(const  std::string &x, ...)
{
	dumpLog(x);
	endLogThread();
	exit(1);
