...
*-------------------------------------------------------------------
*/
#ifndef __RUN_APP_GUI_H__
#define __RUN_APP_GUI_H__
...
namespace app {
	class LIBRUN_EXPORT RunAppGui : public QMainWindow
	{
		Q_OBJECT

	public:
		//all the threads share the same cfg located at only one physical address
		RunAppGui(AppCfgPtr &cfg, QWidget *parent = 0);
		~RunAppGui();
