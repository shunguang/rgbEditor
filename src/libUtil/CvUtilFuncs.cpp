...
void app::myImgWrite(const cv::Mat &x, const std::string &dir, const std::string &fname, const size_t fn, const size_t L)
{
char buf[1024];
...
printf("app::myImgWrite(): cannot create root folder:%s", p0.string().c_str());
}
}

if (!cvSaveImage(fileName, cvTrkImg)) {
printf("app::myImgWrite():  Could not save: %s\n", fileName);
}
else {
printf("app::myImgWrite():  File saved: %s\n", fileName);
}
}
...
void app::drawPlus(cv::Mat &img, const cv::Point2f &pt, const uint32 len, const cv::Scalar& color, int thickness, int lineType, int shift)
{
Point p(pt.x, pt.y);
...
void app::drawPlus(Mat &img, const Point &pt, const uint32 len, const Scalar& color, int thickness, int lineType, int shift)
{
