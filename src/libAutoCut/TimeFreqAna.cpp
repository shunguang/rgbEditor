...
namespace app {
//these global varaiables are defined for supporting FFT be called by multi-thread in parallel.
//call g_fftCreatePlan() before start any thread call TimeFreqAna(), because all thread share the same plan
//call g_fftDestroyPlan() after all threads done()
bool         g_fft_planSet = false;
FFT_PLAN	 g_fftPlan;           //FFT plan
FFT_COMPLEX* g_fftIn=0;           //FFT input data 
FFT_COMPLEX* g_fftOut=0;          //FFT output

void g_fftCreatePlan(const int nFFT) {
if (!g_fft_planSet) {
g_fftIn = new FFT_COMPLEX[nFFT];
g_fftOut = new FFT_COMPLEX[nFFT];
//forward Fourier transform, save the result in 'out'
g_fftPlan = FFT_CREATE_PLAN_1D(nFFT, g_fftIn, g_fftOut, FFTW_FORWARD, FFTW_MEASURE);
g_fft_planSet = true;
}
}
...
TimeFreqAna::TimeFreqAna(const CfgTimeFreqAna &cfg)
: m_cfg(&cfg)
, m_fftIn(NULL)
, m_fftOut(NULL)
//, m_fftPlan(NULL)
...
TimeFreqAna :: ~TimeFreqAna()
{
freeFFT();
}

int TimeFreqAna::process(const RawFrm &x, const float fs, const float hilightSegmentLengthInSec, const std::string &dumpFileBase4Debug)
{
m_threadTag = dumpFileBase4Debug;
int ret = calStfftImg(x);  // cal <m_stfftImg>
if (ret < 0) {
return -1;
}
...
int TimeFreqAna::smoothSpFlux_n_Energy()
{
int nWinSz = 11;
if (nWinSz % 2 == 0) {
nWinSz++;
...
int TimeFreqAna::calCuSum(const swfMat &sp, const float32 a, swfMat &cuSum)
{
uint32 n = sp.size();
swfMat y = sp - a;
...
int TimeFreqAna::locateApplausingSegments(const int nMaxApplausingSegs)
{
...
#if _DEBUG
printf("i=%d, audioRawTimeSeriesIdxRng=[%llu,%llu]\n", i++, seg.idx1, seg.idx2);
#endif
}
return 0;
}
...
int TimeFreqAna::calSpFlux_n_Energy()
{
...
float32 TimeFreqAna::calFrmSpEnergy(const swfMat &sp)
{
float32 se = 0;
#if 0
swfMat sp2 = sp * sp;
...
int TimeFreqAna::calStfftImg(const RawFrm &x)
{
...
//printf("%s, i=%d, nFrms=%d --AA\n", m_threadTag.c_str(), i, nFrms);

fftwf_execute_dft(g_fftPlan, m_fftIn, m_fftOut);
//saveFft(m_fftOut, m_nFFT, "c:/temp/rawFFT", "_frm", i);

//printf("%s, i=%d, nFrms=%d --BB\n", m_threadTag.c_str(), i, nFrms);
...
//output <Y>
void TimeFreqAna::calSpFromFftOutput(float32 *Y, uint32 nSp)
{
#if _DEBUG
...
void TimeFreqAna::setFFTInput(const float32 *x, const uint32 nWinSz)
{
#if _DEBUG
...
uint64 TimeFreqAna::calStfftFrms(const uint64 nBytes)
{
...
#if 0
if (!g_fftIsPlanSet()) {
g_fft_plan_mutex.lock();
g_fftPlan = FFT_CREATE_PLAN_1D(nFFT, m_fftIn, m_fftOut, FFTW_FORWARD, FFTW_MEASURE);
g_fft_planSet = true;
g_fft_plan_mutex.unlock();
}
#endif
...
void TimeFreqAna::fftSetToNull()
{
m_fftIn = 0;
m_fftOut = 0;
}

void TimeFreqAna::printFFT(const FFT_COMPLEX *p, const uint32 n, const int scale, const std::string &msg)
{
...
void TimeFreqAna::normalFFT(const FFT_COMPLEX *p, const uint32 nStart, const uint32 nPts, const char flag)
{
...
void TimeFreqAna::saveFft(const FFT_COMPLEX *fft, const uint32 n, const string &fnameMain, const string &tag1, const int32 num1)
{
string fpath;
...
void TimeFreqAna::printMaxEle(const FFT_COMPLEX *fft, const uint32 n, const char *msg)
{
FFT_ELEMEMT t, x0 = 0;
const FFT_ELEMEMT *p = (FFT_ELEMEMT *)fft;
for (int i = 0; i < n; ++i) {
...
int TimeFreqAna::findFinalAudioSegments(const RawFrm &x, const float fs, const float hilightSegmentLengthInSec, const std::string &dumpFileBase4Debug)
{
