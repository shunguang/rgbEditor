...
QString app::encodeProgressMsg(const ProgressMsg &msg)
{
char buf[1024];
...
ProgressMsg app::decodeProgressMsg(const QString &q)
{
ProgressMsg msg;
...
QImage  app::cvMatToQImage(const cv::Mat &inMat)
{
switch (inMat.type())
{
// 8-bit, 4 channel
...
QPixmap app::cvMatToQPixmap(const cv::Mat &inMat, bool isBGR)
{
if (isBGR) {
...
cv::Mat app::QImage2cvMat(const QImage &image)
{
cv::Mat mat;
qDebug() << image.format();
switch (image.format())
...
app::splitExt(filePath, head, ext);
