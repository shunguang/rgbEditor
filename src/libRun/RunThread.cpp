...
void RunThread::setCfg(AppCfgPtr &cfg)
{
m_cfg = cfg;
}
...
if (pauseJob) {
dumpLog("RunThread::run(): %s -- in sleep mode", m_threadName.c_str());
boost::mutex::scoped_lock lock(m_mutex4Working);
m_condition4Working.wait(lock);
dumpLog("RunThread::run(): %s -- waked up", m_threadName.c_str());
}
...
dumpLog("RunThread::run(): exit loop--%s", m_threadName.c_str());
