...
RunAppGui::RunAppGui(AppCfgPtr &cfg, QWidget *parent)
	: m_cfg(cfg)
	, m_frmNum(0)
	, m_guiReady(false)
	...
	, m_isShowKeyFrm(false)
{
	...
	//toolbar task items start and quit
	...
	//Toolbar combine
	m_pCombineTask = dynamic_cast<ToolbarCombine *> (m_ui->m_vToolbarGui[APP_TOOLBAR_EDIT].get());
	QObject::connect(m_pCombineTask->m_in->m_vPushBotton[COMBINE_IN_PBUTTON_BROWSE_FOLDER], SIGNAL(clicked()), this, SLOT(on_pushBotton_combineBrowseFolder_clicked()), MY_QT_CONN);
	QObject::connect(m_pCombineTask->m_in->m_vPushBotton[COMBINE_IN_PBUTTON_BROWSE_MP3_FILE], SIGNAL(clicked()), this, SLOT(on_pushBotton_combineBrowseMp3_clicked()), MY_QT_CONN);
	QObject::connect(m_pCombineTask->m_in->m_vPushBotton[COMBINE_IN_PBUTTON_LOAD_ALL], SIGNAL(clicked()), this, SLOT(on_pushBotton_combineLoadall_clicked()), MY_QT_CONN);
	QObject::connect(m_pCombineTask->m_in->m_vPushBotton[COMBINE_IN_PBUTTON_REMOVE_ALL], SIGNAL(clicked()), this, SLOT(on_pushBotton_combineRemoveAll_clicked()), MY_QT_CONN);
	QObject::connect(m_pCombineTask->m_in->m_vPushBotton[COMBINE_IN_PBUTTON_APPEND_ONE], SIGNAL(clicked()), this, SLOT(on_pushBotton_combineBrowseAppendOne_clicked()), MY_QT_CONN);
	QObject::connect(m_pCombineTask->m_in->m_vPushBotton[COMBINE_IN_PBUTTON_REMOVE_ONE], SIGNAL(clicked()), this, SLOT(on_pushBotton_combineBrowseRemoveOne_clicked()), MY_QT_CONN);
	QObject::connect(m_pCombineTask->m_in->m_vPushBotton[COMBINE_IN_PBUTTON_UP], SIGNAL(clicked()), this, SLOT(on_pushBotton_combineUp_clicked()), MY_QT_CONN);
	QObject::connect(m_pCombineTask->m_in->m_vPushBotton[COMBINE_IN_PBUTTON_DOWN], SIGNAL(clicked()), this, SLOT(on_pushBotton_combineDown_clicked()), MY_QT_CONN);
...
void RunAppGui::respns_toolbarSwitchTask( const AppToolbarTask newTask )
{
	m_cfg->setTask(newTask);
	m_ui->switchToolbarTaskTo(newTask );
}

void RunAppGui::respns_toolbarStartTask(const AppToolbarTask newTask)
{
}
void RunAppGui::respns_toolbarQuitTask(const AppToolbarTask newTask)
{
...
void RunAppGui::on_actionHelp_triggered()
{
...
void RunAppGui::on_actionAbout_triggered()
{
	QMessageBox::information(this, POPUP_MSG_WIN_TITLE, "          Auto Clip Cut\n          Shunguang Wu 2018.         ");
}

void RunAppGui::on_actionExit_triggered()
{
	...
	dumpLog("RunAppGui::on_actionExit_triggered()--B");
...
void RunAppGui::closeEvent(QCloseEvent *event)
{
...
void  RunAppGui::showDebugMsg(const char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
...
void RunAppGui::showDebugMsg(const std::string &msg)
{
	QString s("");
...
void RunAppGui::on_pushBotton_combineBrowseFolder_clicked()
{
...
void RunAppGui::on_pushBotton_combineBrowseMp3_clicked()
{
}

void RunAppGui::on_pushBotton_combineLoadall_clicked()
{
}

void RunAppGui::on_pushBotton_combineRemoveAll_clicked()
{
}

void RunAppGui::on_pushBotton_combineBrowseAppendOne_clicked()
{
}

void RunAppGui::on_pushBotton_combineBrowseRemoveOne_clicked()
{
}

void RunAppGui::on_pushBotton_combineUp_clicked()
{
}

void RunAppGui::on_pushBotton_combineDown_clicked()
{
}

void RunAppGui::on_lineEdit_inputVideoFolder_edited(const QString &s)
{
	...
	void RunAppGui::on_pushBotton_removeAll_clicked()
{
	m_ui->removeAllInputVideos();
}

void RunAppGui::on_pushBotton_removeOne_clicked()
{
	int idx;
	string fname;
...
void RunAppGui::on_pushBotton_addAll_clicked()
{
...
void RunAppGui::on_pushButton_startProcess_clicked()
{
...
void RunAppGui::on_pushButton_upload_clicked()
{
...
void RunAppGui::on_pushButton_reset_clicked()
{
...
void RunAppGui::on_pushButton_quit_clicked()
{
	respns_stop();
	on_actionExit_triggered();
}

void RunAppGui::on_pushBotton_addGameVideo_clicked()
{
	CfgGame &g = m_cfg->m_vGames[0];
	QString initFolder = QString::fromStdString(g.inputVideoFolder);
...
//Game info group_box
void RunAppGui::on_lineEdit_gameName_edited(const QString &s)
{
...
void RunAppGui::on_dateEdit_gameDate_changed(const QDate &d)
{
	std::string mmddyyyyy = d.toString().toStdString();
	m_cfg->updateGameDate(mmddyyyyy);
}

void RunAppGui::on_lineEdit_gameScore_edited(const QString &s)
{
...
void RunAppGui::on_pushBotton_browseInputVideoFolder_clicked()
{
	CfgGame &g = m_cfg->m_vGames[0];
	QString initFolder = QString::fromStdString(g.inputVideoFolder);
	QString dir = QFileDialog::getExistingDirectory(this, tr("Open Input Video Folder"), initFolder,
...
void RunAppGui::on_tableWidget_clicked(const QModelIndex &idx)
{
	int fileId = -1;
	if (idx.isValid()) {
		fileId = idx.row();
...
void RunAppGui::on_pushBotton_addOne_clicked()
{
	const CfgGame &g = m_cfg->m_vGames[0];
	const string &folder = g.inputVideoFolder;
	boost::filesystem::path p(folder);
	...
	string head, fname;
	splitFolder(ff.toStdString(), head, fname);
...
void RunAppGui::on_lineEdit_gameMp3Path_edited(const QString &s)
{
	std::string f = s.toStdString();
	m_cfg->updateMp3File(f);
}

void RunAppGui::on_pushBotton_addMp3File_clicked()
{
...
//ouput group_box
void RunAppGui::on_lineEdit_outputVideoFolder_edited(const QString &s)
{
	//crashed on  dir.toStdString()
	std::string dir2 = s.toStdString();
...
void RunAppGui::on_pushBotton_browseOutputVideoFolder_clicked()
{
...
void RunAppGui::on_comboBox_outputVideoFileFmt_currentIndexChanged(int idx)
{
	int newIdx = idx;
	m_cfg->updateOutputVideoFmt(newIdx);
}

void RunAppGui::on_comboBox_outputVideoFrmSize_currentIndexChanged(int idx)
{
	m_cfg->updateOutputVideoFrmSize(idx);
}
void RunAppGui::on_comboBox_outputVideoFrmRate_currentIndexChanged(int idx)
{
	m_cfg->updateOutputVideoFrmRate(idx);
}

void RunAppGui::on_comboBox_uploadPlatform_currentIndexChanged(int idx) {
	m_cfg->updateUploadPlatform(idx);
}

//--- Core parameters
void RunAppGui::on_comboBox_methods_currentIndexChanged(int idx)
{
...
void RunAppGui::on_keyFrmSlider_valueChgd(int keyFrmIdx)
{
...
void RunAppGui::on_keyFrmSlider_released()
{
...
void RunAppGui::on_keyFrmSlider_pressed()
{
	m_isShowKeyFrm = true;
	m_runMgr->stopDecoding();
}

void RunAppGui::on_keyFrmSlider_moved()
{
	//todo:
}


void RunAppGui::on_pushbutton_addSlider_clicked()
{
...
void RunAppGui::on_pushbutton_delSlider_clicked()
{
...
void RunAppGui::respns_resizeGui()
{
	m_ui->resizeUI();
}

void RunAppGui::respns_jobDone()
{
...
void RunAppGui::respns_warning_msg(const QString &msg)
{
	popupInfoMsg(POPUP_MSG_WIN_TITLE, msg.toStdString(), 5000 /*, QRect *placeRect = NULL*/);
}

void RunAppGui::respns_mgrHasErr(const QString &errMsg)
{
	popupMsg(this, POPUP_MSG_WIN_TITLE, errMsg.toStdString());
}

void RunAppGui::respns_mgrProgress_msg(const QString &msg)
{
	...
	dumpLog(msg.toStdString());
}

void RunAppGui::respns_mgrAllDexed(const QString &msg)
{
	dumpLog("RunAppGui::respns_mgrAllDexed()--AAA");
...
void RunAppGui::respns_mgrWrtOutputDone(const QString &msg)
{
...
void RunAppGui::respns_mgrAllAnalyzed(const QString &msg)
{
	m_currFileId = 0;
...
void RunAppGui::popupAutoDispearMsg(const std::string &msg)
{
	QRect rect(10, m_ui->getGuiRect().bottom() - 120, 400, 100);
...
void RunAppGui::updateCurrVideoGuiWhenSwitchFiles(int fileId)
{
	//RunMgr *p = dynamic_cast<RunMgr *> (m_mgrThread.get());
...
void RunAppGui::respns_previous()
{
...
void RunAppGui::respns_changeVolume(int v)
{
}

void RunAppGui::respns_changeMuting(bool flag)
{
}

void RunAppGui::respns_changePlayFrmRate(qreal r)
{
	m_runBrowseVideo->setFrmRate(r * 30);
}

//todo
uint32_t RunAppGui::getPrevKeyFrmIdx()
{
	return 0;
}

uint32_t RunAppGui::getNextKeyFrmIdx()
{
	return m_currVideoTotalKeyFrms;
}


void RunAppGui::respns_dispImg()
{
...
#if 0
void RunAppGui::dispImg(cv::Mat &I)
{
	QPixmap I2 = app::cvMatToQPixmap(I);
	//I2.save("c:/temp/imgToshow2.png", "PNG");
	m_ui->showImg(I2);
}
#endif

void RunAppGui::dispImg(const DispFrm *p)
{
	...
	QPixmap I = app::cvMatToQPixmap(p->m_img);
